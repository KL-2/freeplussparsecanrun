...
def main(unused_argv):
  ...
  def lpips_fn(x, y):
    ...
    score = lpips_vgg(torch.from_numpy(np.array(x)).permute(2, 0, 1).unsqueeze(0), 
                      torch.from_numpy(np.array(y)).permute(2, 0, 1).unsqueeze(0))                      
    return score.item()
  census_fn = jax.jit(
  ...
  for idx in range(dataset.size):
    ...
    if not config.render_path:
      ...
      #mse = ((rendering['rgb'] - batch['rgb'])**2).mean()
      #print(f"render:{rendering['rgb']}and batch:{batch['rgb']}")
      #if mse > 0:
       #   psnr = math.mse_to_psnr(mse)
       #   metric['psnr'] = float(psnr)
      #else:
    # Handle the case when MSE is zero or negative
         # metric['psnr'] = 0.0  # Or any appropriate value
