...
def train_step(
  ...
  Args:
    ...
    rng: jnp.ndarray, random number generator.随机数生成器
    state: utils.TrainState, state of the model/optimizer.
    batch: dict, a mini-batch of data for training.4096会CUDA OOM
...
#   print('step:', step)
  def loss_fn(variables): # contains global vars of train_step, such as batch...
    ...
    #这样计算的 L2 范数通常用于权重衰减的目的，即通过在损失函数中添加一个正则化项，
    #以降低模型复杂度，避免过拟合。L2 范数正则化项的形式通常是权重衰减乘以参数的 L2 范数。

    ## -- add ray_history as output for occ_reg（遮挡正则化）or distortion loss -- ##
    renderings, ray_history = model.apply(  #前向传播#ray_history多于sparsenerf@
    ## -------------------------------------------------------------- ##
        variables,
        ...
        freq_reg_mask=batch['freq_reg_mask'] if config.freq_reg else None)#频率正则化
        #多于sparsenerf@
    
    lossmult = batch['rays'].lossmult # 多尺度损失
...
#sparsenerf相较于freenerf添加的内容
      # depth loss  重点多的depth的位置￥
      rays_per_superpixels = 4
      depth = rendering['distance_mean']
      depth = depth.reshape(-1,rays_per_superpixels).transpose()# two pairs of points, so here is 4
      #（1，M）->(4,M/4)
      ...
      if config.compute_disp_metrics:
        # Using mean to compute disparity, but other distance statistics can be used instead.
    ...
    if render_random_rays:
      ...
      renderings_random, _ = model.apply(
          ...
          resample_padding=resample_padding,
          compute_extras=True,
          ## ---- add freq_reg_mask as input ---- ## #多于sparsenerf @
    ...
    loss = (
        losses[-1] + config.coarse_loss_mult * jnp.sum(losses[:-1]) + +0.5*all_depth_loss[-1]+
...
def main(unused_argv):
  ...
  #使用JAX的随机数生成器密钥rng和NumPy的随机数生成器来设置全局的随机数种子，
  #以确保在不同的主机上加载数据时引入一定的随机性，使得训练过程中的随机性更为多样化。
  #这对于分布式计算和多GPU训练是有用的，因为不同主机或GPU上的数据加载和计算可能会有一些不同，
  #通过引入随机性可以减少同步问题并提高训练的稳定性
  ...
  dataset = datasets_depth.load_dataset('train', config.data_dir, config)
  test_dataset = datasets_depth.load_dataset('test', config.data_dir, config)
  ...
  for step, batch in zip(range(init_step, config.max_steps + 1), pdataset):
    ## ---- a tricky way to use freq_reg_mask ------------- ###多于sparsenerf
    if config.freq_reg:
      batch['freq_reg_mask'] = (
    ...
    if config.train_render_every > 0 and step % config.train_render_every == 0:
      if config.freq_reg:#多于sparsenerf
