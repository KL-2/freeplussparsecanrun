...
class Config:
  ...
  batch_size: int = 512  # The number of rays/pixels in each batch.4096
  batch_size_random: int = 512  # The number of rays/pixels in each batch.4096
  ...
  # weight_decay_mult: float = 1.0  # The multiplier on weight decay.
  ...
  render_chunk_size: int = 1024  # Chunk size for whole-image renderings.#4096
  ...
  sampled_pairs: int = 10

  # New loss function weights
  depth_tvnorm_loss_mult: float = 0.1  # Loss weight of depth tv norm.#0.0
  ...
  train_render_every: int = 1000  # Steps between test set renders for training
  # train_render_every: int = 5  # Steps between test set renders for training
  ...
  dtu_pose_path:  str = 'posh_path' # pose path, only for dtu

  ###########
  random_pose_flag: bool = False
...
def define_common_flags():
  ...
  flags.DEFINE_string('checkpoint_dir', "sparse", 'Gin config files.')
...
# config = configs.load_config(checkpoint_dir=args.checkpoint_dir, postfix=args.postfix, dataset_id=args.dataset_id)
def load_config(save_config=True):
  ...
  if True:
    ...
    gin.parse_config_file(flags.FLAGS.gin_configs)
  config = Config()
  config.dtu_scan = flags.FLAGS.dataset_id
  config.llff_scan = flags.FLAGS.dataset_id  
  config.n_input_views = int(flags.FLAGS.n_input_views)#覆盖掉llff3.gin中的
  if True:
    config.checkpoint_dir = 'out/'+flags.FLAGS.checkpoint_dir+flags.FLAGS.dataset_id+flags.FLAGS.postfix
  else:
    config.checkpoint_dir = flags.FLAGS.checkpoint_dir
  
  if save_config and jax.host_id() == 0:
    ## ----- modify to support better logging -------- ##
    # os.makedirs(config.checkpoint_dir)
    if not exists(config.checkpoint_dir):
      os.makedirs(config.checkpoint_dir)    
