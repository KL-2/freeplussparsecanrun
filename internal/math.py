...
#提供对具有权重的数据集的百分位数估计，确保按照给定的权重对数据进行考虑。
...
#通过比较像素及其邻域像素之间的差异，为图像中的每个像素计算一个三值编码，用于表示其周围的局部结构信息。
...
## ------------------------ FreeNeRF add-ons -------------------------- ##
    # Inter-Interval Loss（区间间损失）：计算不同区间之间的损失。
    #对于给定的区间，计算了该区间与所有其他区间之间的绝对差的加权和。这一部分损失衡量了不同区间之间的失真。

    # Intra-Interval Loss（区间内损失）：计算每个区间内的损失。
    #对于给定的区间，计算了该区间内部所有点之间的绝对差的加权和。这一部分损失衡量了区间内部的失真。
...
def get_freq_reg_mask(pos_enc_length, current_iter, total_reg_iter, max_visible=None, type='submission'):
  ...
  #@freenerf positional encoding 模拟退火
