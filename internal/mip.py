...
# def lossfun_distortion(t_mids, u_diff, w):
#   """Compute iint w[i] w[j] |t[i] - t[j]| di dj."""
#   # The loss incurred between all pairs of intervals.
#   # ut = (t[..., 1:] + t[..., :-1]) / 2
#   dut = jnp.abs(t_mids[..., :, None] - t_mids[..., None, :])
#   # print('###### w.shape, dut.shape:', w.shape, dut.shape) # (12, 128) (12, 127, 127) 
#   loss_inter = jnp.sum(w * jnp.sum(w[..., None, :] * dut, axis=-1), axis=-1)

#   # The loss incurred within each individual interval with itself.
#   # loss_intra = jnp.sum(w**2 * (t[..., 1:] - t[..., :-1]), axis=-1) / 3
#   loss_intra = jnp.sum(w**2 * u_diff, axis=-1) / 3
...
def volumetric_rendering(rgbs, weights, normals, t_vals, white_background,
  ...
  #获取相邻深度值的平均值
  t_diff = t_vals[Ellipsis, 1:]-t_vals[Ellipsis, :-1]
  ...
  # rendering['dist_loss'] = lossfun_distortion(t_mids, t_diff, weights)
  ...
  if True:
    ...
    distance_mean  = (
        jnp.clip(
            ...
            t_vals[:, -1]))
    if True:
      rendering['distance_mean'] = distance_mean


  if True:
    eps = 1e-10
    rgb_combine = weights*(rgbs.sum(axis=-1)/3.0)
